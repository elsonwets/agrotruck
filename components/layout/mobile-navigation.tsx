...
const links = [
  ["nav.companies", "/entreprises"], ["nav.pricing", "/pricing"], ["nav.how", "/comment-ca-marche"], ["nav.about", "/about"],
] as const;
export function MobileNavigation({ authenticated, admin }: { authenticated: boolean; admin: boolean }) {
  ...
  return <div className="lg:hidden">
    <div className="flex items-center gap-2"><div className="hidden sm:block"><HeaderControls/></div>{admin&&<AdminNotificationBell/>}<button className="focus-ring grid size-11 place-items-center rounded-xl border border-primary/15 bg-primary/5 text-primary" onClick={() => setOpen(true)} aria-label={t("menu.open")} aria-expanded={open} aria-controls="mobile-navigation-dialog"><Menu/></button></div>
