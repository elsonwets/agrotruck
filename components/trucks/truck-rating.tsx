...
export function TruckRating({ ratings: baseRatings, slug, detailed = false, className }: Props) {
  ...
  useEffect(() => {
    if (!detailed) return;
    fetch(`/api/reviews/${encodeURIComponent(slug)}`).then((response) => response.ok ? response.json() : null).then(setVisitorRatings).catch(() => undefined);
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch(`/api/reviews/${encodeURIComponent(slug)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ visitorId, ...scores, website: "" }) });
