...
export function TrucksBrowser({ initialTrucks, mode }: { initialTrucks: Truck[]; mode?: "rental" | "sale" }) {
  ...
  const results = useMemo(() => initialTrucks.filter((truck) =>
    ...
    && (!filters.mode || truck.listingMode === filters.mode)
  ), [filters, initialTrucks]);

  const reset = () => setFilters({ ...emptyFilters, mode: mode ?? "" });

  return <>
    <div className="mb-7 rounded-[22px] border border-primary/10 bg-white p-4 shadow-[0_18px_50px_rgba(17,17,17,.06)] sm:p-5">
      <TruckFilters value={filters} onChange={setFilters} onReset={reset} trucks={initialTrucks} lockedMode={mode} />
    </div>
